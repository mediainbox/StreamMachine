...
module.exports = class WorkerPool extends require("events").EventEmitter
    constructor: (@slave,@size,@config) ->
        @workers    = {}
        @_shutdown  = false

        debug "Worker pool init with size of #{@size}."

        @log = @slave.log.child component:"worker_pool"
    ...
    _spawn: ->
        ...
        w.rpc = new RPC p, functions:
            ...
            send_listener: (msg,handle,cb) =>
                @slave._listenerFromWorker w.id, msg, handle, cb
